...
- (LAFImportResult)importHeader:(NSString *)header {
    return [self addImport:header];//[self importStatementFor:header]
}

- (BOOL)hasImportedHeader:(NSString *)header {
    [self cacheImports];//更新缓存
    return [_importedCache containsObject:header];//[self importStatementFor:header]
}
...
+ (void)showAboveCaretOnCenter:(NSString *)text color:(NSColor *)color {
    ...
    [field setBordered:NO];
    [field setEditable:NO];
    field.frame = CGRectOffset(field.frame, -field.bounds.size.width/2, - field.bounds.size.height/2);
